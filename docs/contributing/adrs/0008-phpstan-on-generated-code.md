...
3. **The analysis runs over committed `expected/` trees**, not `generated/`:
   ...
   tests already guarantee the two are identical. Fixtures holding deliberately
   partial trees (`runtime-boilerplate`) are excluded, since their missing
   runtime classes are false positives — those templates are already analysed as
   part of `src/`.
4. **Coverage is every fixture, not a sample.** A gate over a curated subset
   lets the rest regress silently. `tools/phpstan-generated.php` discovers the
   trees, so a new fixture is analysed the moment it is committed.
5. **Every fixture generates into its own namespace** (its config appends a
   segment derived from the fixture directory name), guarded by
   `ExpectedNamespaceUniquenessTest`. PHPStan resolves duplicate class names
   against an arbitrary declaration, which makes analysing colliding fixtures
   in one process meaningless — and the collision cannot be predicted from the
   directory layout: before namespaces were made unique, the OpenApi2
...
6. **Fixtures are analysed in file-count-capped groups.** One PHPStan process
   over all ~20 000 files is OOM-killed (exit 137) rather than failing loudly,
   locally as much as on a GitHub runner. `tools/phpstan-generated.php` packs
   fixtures into groups of at most 4 000 files (a fixture larger than the cap
   runs alone), and `castor qa:phpstan:generated` runs one process per group
   with `--memory-limit=4G`, so an oversized group fails visibly.
7. **Pre-existing findings are frozen** in `phpstan-generated-baseline.neon`,
   so the gate blocks new regressions immediately instead of waiting on a
   cleanup. The baseline is regenerated (never hand-edited) as generator fixes
   shrink it: each group writes its own baseline and they are concatenated
   verbatim — entries are never parsed or reformatted, because PHPStan writes
   multi-line messages as NEON block strings in which blank lines are
   significant. Since every group only ever sees its own share of the entries,
   the config sets `reportUnmatchedIgnoredErrors: false`; stale entries are
   swept out on regeneration rather than reported live.

## Consequences

- New generator work cannot introduce type errors into shipped output without
  the CI job going red.
- The baseline is a work list, not a resting place: its entries collapse into a
  handful of systematic generator patterns, each fixable on its own.
- `ConstraintReferencesTest`, which hand-rolls a regex hunting for referenced
  ...
  retired now that every fixture is analysed.
