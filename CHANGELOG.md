...
## [Unreleased]
### Added
- [Tests] Nightly, non-blocking **corpus smoke job** (`castor qa:corpus`, `.github/workflows/corpus-smoke.yml`): generates a client for each vendor-published real-world spec pinned in `corpus/specs.json` (Kubernetes, Docker, Slack, Netlify, GitHub 3.0 / 3.1, Stripe, Plaid, Box, Twilio, Discord, OpenAI, Cloudflare) and asserts that generation does not crash and the output parses, with a report-only Mago summary — no committed output or baselines ([#1041](https://github.com/janephp/janephp/issues/1041) point 3). The fixture tests' php-parser syntax gate moved into the reusable `PhpSyntaxGate`
- [JsonSchema] [OpenApi] Console `generate` commands now print generation progress: a header per schema (`Generating for schema …` / `Output: …`), timed `➜ Guessing… done (Xs)` / `➜ Generating… done (Xs)` phase lines, a `➜ N files written` total, the grouped file counts (models, enums, normalizers, validators, runtime, auth, client, endpoints, exceptions) and `Done in Xs`. Silent at `--quiet` ([#252](https://github.com/janephp/janephp/issues/252), [ADR 0013](docs/contributing/adrs/0013-generation-events.md))
- [JsonSchema] [OpenApi] New **experimental** generation events system ([#859](https://github.com/janephp/janephp/issues/859), [ADR 0013](docs/contributing/adrs/0013-generation-events.md)), built on `symfony/event-dispatcher` (new require of `jane-php/json-schema` and `jane-php/open-api-common`). Subscribers implement `Symfony\Component\EventDispatcher\EventSubscriberInterface`; `Jane::build()` / `JaneOpenApi::build()` accept an optional second `Symfony\Contracts\EventDispatcher\EventDispatcherInterface` argument — when omitted, a listener-less dispatcher is used. Lifecycle events (`GenerationStarted/EndedEvent`, `SchemaStarted/EndedEvent`, `GuessingStarted/EndedEvent`, `GeneratingStarted/EndedEvent`, `FileGeneratedEvent`) carry readonly payloads; mutation events carry **live mutable references**: `PropertyGuessedEvent` lets listeners replace a guessed property type (`Property::setType()`) so models and normalizers stay consistent, and `PropertyGeneratedEvent` / `ClassGeneratedEvent` expose the model's PhpParser AST (`Stmt\Property` / `Stmt\Class_`) for docblock types, psalm tags, custom methods, traits or extends. Event constructors are frozen; the event set may grow. Throwing listeners abort generation through the existing `GenerationFailedException` wrapping
### Removed
- [OpenApi] New `x-fetch-mode` OpenAPI extension on **GET and HEAD** operations (values `lazy` — the default, `eager`, `preload`), plus the `default-fetch-mode` generation option (mirrors `default-additional-properties` plumbing, GET/HEAD only). `lazy` returns a lazy ghost proxy deferring the send to first access; `eager` is the historical blocking behavior; `preload` registers the request immediately. Every other verb is always eager, and misuses (`x-fetch-mode` on a non-GET/HEAD operation, invalid values) fail generation with clean, complete `[ERROR]` blocks through a new `FetchModeValidator` plugged into each component's `SchemaParser::validateSchema()` ([ADR 0002](docs/contributing/adrs/0002-pre-generation-schema-validation.md), [ADR 0012](docs/contributing/adrs/0012-symfony-httpclient-migration-x-fetch-mode.md))
- [OpenApi] `lazy` / `preload` operations return a **lazy ghost proxy** of the endpoint's generated model class built on PHP native lazy objects (PHP >= 8.4; earlier versions degrade to the eager behavior at runtime). Reading any property of the proxy sends the request, parses the response through the endpoint's own status-code mapping and copies the parsed model's properties onto the proxy (`instanceof`, cloning and serialization behave like a plain model); documented 4xx/5xx exceptions are thrown on first access. Generated endpoints expose `getTargetClass(): ?string`; responses that are not a single generated model (JSON arrays and maps, scalar bodies, no-content responses, several models / content types) are not ghostable and their configured deferred mode degrades to eager (see [ADR 0014](docs/contributing/adrs/0014-fetch-mode-ghost-proxies.md)). New runtime helper `Jane\Component\OpenApiRuntime\Client\GhostFactory` (availability check, ghost creation, forced initialization)
- [OpenApi] New `Jane\Component\OpenApiRuntime\Client\Plugin\ServerUrlHttpClient` decorator replacing `AddHostPlugin` + `AddPathPlugin`: request URLs are explicitly rewritten to the specification's server URL (scheme, host, port, base path). `base_uri` cannot be used for this: RFC 3986 resolution drops the base URI path for absolute-path request URLs (verified against symfony/http-client)
### Removed
- **BC-breaking** [OpenApi] Remove the `Jane\Component\OpenApiRuntime\Client\Result` wrapper and its surface (`toObject()`, `toArray()`, `getStatusCode()`, `isInitialized()`, `await()`, `cancel()`): `lazy` / `preload` operations return a lazy ghost proxy of the model class instead (see the Added entries and [ADR 0014](docs/contributing/adrs/0014-fetch-mode-ghost-proxies.md)); state introspection uses the reflection API (`ReflectionClass::isUninitializedLazyObject()` / `initializeLazyObject()`), batching uses `executeRawEndpoint()` + `stream()` on raw responses, and custom `Endpoint` implementations must add the new `getTargetClass(): ?string`
- **BC-breaking** [JsonSchemaRuntime] Remove the global static state from `Jane\Component\JsonSchemaRuntime\Reference` (`allowExternalRefs()`, `setAllowedExternalHosts()`, `setAllowedLocalRefRoots()`, `setFollowRedirects()`, `resetConfig()` and the process-wide caches). Reference configuration now lives on a `ReferenceResolver` instance threaded through the generation pipeline; generated code keeps resolving references through a shared `ReferenceResolver::default()` ([ADR 0008](docs/contributing/adrs/0008-instance-owned-reference-resolution.md))
- **BC-breaking** [JsonSchema] Remove the deprecated `Jane\Component\JsonSchema\Exception\JaneExceptionInterface` alias; the JsonSchema and OpenAPI exceptions now implement `Jane\Component\JsonSchemaRuntime\Exception\JaneExceptionInterface` directly
- **BC-breaking** [OpenApi] Remove the deprecated `$fetch = 'response'` mode and the `FETCH_RESPONSE` constant from the generated client: `executeEndpoint()` always parses the response body, and `executeRawEndpoint()` remains the explicit raw-response escape hatch. The `trigger_deprecation` bridge is gone
- **BC-breaking** [OpenApi] Rename the mis-spelled anyOf/oneOf reference guessers (`AnyOfReferencefGuesser`, `OneOfReferencefGuesser`) to `AnyOfReferenceGuesser` / `OneOfReferenceGuesser`; the shared logic now lives in `OpenApiCommon\Guesser\OpenApiSchema\AbstractXOfReferenceGuesser`
- **BC-breaking** [OpenApi] Remove PSR-7, PSR-17, PSR-18 and HTTPlug from the OpenAPI runtime and generated clients: the runtime and generated code are now built on `Symfony\Contracts\HttpClient\HttpClientInterface` / `Symfony\Contracts\HttpClient\ResponseInterface` and native `string|resource|array` bodies. `jane-php/open-api-runtime` requires `symfony/http-client ^6.4 || ^7.0 || ^8.0` and no longer depends on `psr/*`, `php-http/*` or `nyholm/psr7`. The HTTPlug `PluginClient` / `AddHostPlugin` / `AddPathPlugin` stack is replaced by `HttpClientInterface` decorators (see the Added / Changed entries and [ADR 0012](docs/contributing/adrs/0012-symfony-httpclient-migration-x-fetch-mode.md))
- **BC-breaking** [OpenApi] Remove the legacy `$fetch` / `FETCH_OBJECT` mechanism: generated operation methods lost their trailing `string $fetch` parameter, `Client::FETCH_OBJECT` and the vestigial `$fetchMode` argument of `parseResponse()` are gone, and the dead `Jane\Component\OpenApiRuntime\Client\Exception\InvalidFetchModeException` is deleted. Raw responses: `executeRawEndpoint()`; per-operation fetch strategies: `x-fetch-mode` (below)
- **BC-breaking** [OpenApi] Remove `Client::create()` dependencies on the PSR-17 factories: the generated signature is now `create(?Symfony\Contracts\HttpClient\HttpClientInterface $httpClient = null, array $additionalPlugins = [], array $additionalNormalizers = [][, bool $applyServerPlugins = true])` and the constructor is `__construct(HttpClientInterface $httpClient, SerializerInterface $serializer)`

### Changed
- **BC-breaking** [OpenApi] `throw-unexpected-status-code` is now enabled by default: a response status (or content type) the specification does not declare makes the generated endpoint throw `BadResponseException` (an `UnexpectedStatusCodeException` carrying the response) instead of returning `null` from `transformResponseBody()` — the behaviour announced in [#377](https://github.com/janephp/janephp/issues/377) and asked for again in [#815](https://github.com/janephp/janephp/issues/815). Set `'throw-unexpected-status-code' => false` to keep the old outcome; the generated method then ends in an explicit `return null;` instead of falling through. `@return` no longer lists `null` when the option is on ([#1066](https://github.com/janephp/janephp/issues/1066), `missing-return-statement`)
- **BC-breaking** [OpenApi] Generated endpoint constructors and client operation methods no longer default `$queryParameters`, `$formParameters` and `$headerParameters` to `[]` when the specification requires one of their keys: the options resolver rejected the empty array at request time anyway, which with the lazy default fetch mode only surfaced on first access of the `Result`. The signature now matches the documented `array{...}` shape and a missing array fails at the call site (`ArgumentCountError`) instead of later (`MissingOptionsException`). An optional request body declared before such an array loses its `= null` default as well, as PHP already treated it as required ([#1066](https://github.com/janephp/janephp/issues/1066), `invalid-parameter-default-value`)
- **BC-breaking** [JsonSchema] [OpenApi] Generation options are consolidated into a single frozen `Jane\Component\JsonSchema\Generator\Options` value object as the canonical source of defaults. Programmatic callers that previously had to pass a required `reference` key to `JsonSchema\Jane::build()` no longer do (it now defaults to `false`); `reference` defaults to `false` uniformly. The console keeps its historical `reference => true` default. ([ADR 0009](docs/contributing/adrs/0009-generation-options-value-object.md))
- **BC-breaking** [OpenApi3] [OpenApi31] Generated status-level exception classes now use constructor property promotion (`private readonly`) instead of a property block plus constructor assignments
- [JsonSchema] [OpenApi] Reference resolution, the validator factory and the schema parser no longer use process-global static state; each build owns its instance ([ADR 0008](docs/contributing/adrs/0008-instance-owned-reference-resolution.md))
- [OpenApi3] [OpenApi31] Consolidate 100%-identical generator code (request-body content generators, `GetTransformResponseBodyTrait`, `GetGetExtraHeadersTrait`) into `OpenApiCommon` behind version hooks ([ADR 0010](docs/contributing/adrs/0010-openapi-3-31-generator-consolidation.md))
- [JsonSchema] [OpenApi] Console commands now declare their name and description through the `#[AsCommand]` attribute (command names unchanged), the `--config-file` option value is validated before reaching the typed loaders, and `Application::VERSION` now derives from Composer's installed versions instead of a stale constant
- [JsonSchemaBundle] [OpenApiBundle] Bundle commands are fully DI-managed (autowired loaders, `#[AsCommand]`, delegation through the component command lifecycle with real exit codes) and resolve their default configuration from `%kernel.project_dir%`: the documented `config/jane/json-schema.php` / `config/jane/open-api.php` paths are now honored first, with the legacy underscored variants as fallback. Configuration under `jane_json_schema:` / `jane_open_api:` is validated: empty config is accepted, unknown keys now fail loudly instead of being silently ignored. Both bundles now require `symfony/config`, `symfony/console`, `symfony/dependency-injection` and `symfony/http-kernel`, and the PHP floor is `^8.1`
- [JsonSchema] `php-cs-fixer` run during generation no longer swallows its output and exit code: fixer output is forwarded at `-v` verbosity and a failing fixer run now triggers a warning and an exception
- **BC-breaking** [OpenApi] HTTP client decorators replace HTTPlug plugins: `$additionalPlugins` in the generated `Client::create()` now accepts **decorator factories** (`callable(HttpClientInterface): HttpClientInterface`, applied left-to-right after the server URL decorator). `Jane\Component\OpenApiRuntime\Client\Plugin\AuthenticationRegistry` is now such a decorator factory reading authentication scopes from the request `extra` option bag (`AuthenticationRegistry::SCOPES_OPTION`, the historical `X-Jane-Authentication` header is gone), and generated authentication classes implement `decorate(string $method, string $url, array &$options): void` + `getScope(): string` instead of `authentication(RequestInterface): RequestInterface` (header based authentication writes to `$options['headers']`, query based to `$options['query']`)
- **BC-breaking** [OpenApi] Multipart request bodies are built by a new internal `Jane\Component\OpenApiRuntime\Client\MultipartStreamBuilder` (raw string body) replacing the PSR-7 based `php-http/multipart-stream-builder`, keeping the GH#1036 part semantics (filename fallback to the property name, per-part `Content-Type` from `encoding`). Binary request bodies are typed `string|resource` (PSR-7 `StreamInterface` is no longer accepted); `BaseEndpoint` no longer depends on a `StreamFactoryInterface`
- **BC-breaking** [OpenApi] No-throw parity with PSR-18 is now implemented through Symfony's per-method `$throw` flags: the runtime consumes responses with `getHeaders(false)` / `getContent(false)` so status-code mapping stays in the generated code. The historical `'throw' => false` request option is not used (it no longer exists in symfony/http-client 8.x); raw responses returned by `executeRawEndpoint()` keep Symfony's default `$throw = true` semantics on explicit reads
- [OpenApi] Redirects keep Symfony's default (`max_redirects: 20`), matching the historical HTTPlug behavior
...
## [7.14.0] - 2026-08-31
### Added
- [JsonSchemaRuntime] The reference resolution exception hierarchy now distinguishes failure causes. Beyond `ReferenceResolveException` (the fetch / decode base), `ReferenceFetchException`, `ReferenceDecodeException`, `ReferencePointerException` and `InvalidReferenceException` are thrown for the matching failure mode, all still extending `\RuntimeException` (and implementing `JaneExceptionInterface`). Foreign exceptions from `league/uri`, `Rs\Json\Pointer` and `symfony/yaml` are wrapped with the failing reference / pointer in the message and their original exception chained as `getPrevious()`
- [JsonSchemaRuntime] `Reference::resolve()` no longer allocates an identity closure when no deserialize callback is passed, and the `league/uri` v6-vs-v7 capability sniff is cached per-process instead of running `method_exists()` on every `Reference` construction
- [OpenApi] The shipped `CustomQueryResolver` interface types its `$value` parameter as `mixed` (narrowing nothing); `Endpoint::getBody()`, the `BaseEndpoint` abstract declaration and `getMultipartBody()` accept an optional `?Psr\Http\Message\StreamFactoryInterface` instead of an untyped `$streamFactory`; `transformResponseBody` carries a shared `@return mixed` docblock on both its declarations
...
- [JsonSchemaRuntime] `ReferenceResolveException` (unfetchable / unparsable reference documents) and the new `MalformedJsonException` (malformed JSON response body in generated endpoints) now join the Jane error taxonomy: they implement the `JaneExceptionInterface` marker, which moved to `Jane\Component\JsonSchemaRuntime\Exception\` so runtime exceptions can be user-facing without inverting the component dependency direction. Generation commands render these failures as clean `[ERROR]` blocks per [ADR 0002](docs/contributing/adrs/0002-pre-generation-schema-validation.md) (the historical `Jane\Component\JsonSchema\Exception\JaneExceptionInterface` alias is removed in the 8.x major)
...
### Changed
- **BC-breaking** [JsonSchema] [OpenApi] Generated models now expose **public, natively typed properties** instead of protected properties with `getXxx()` / `setXxx()` accessors. Optional / nullable properties are typed `?type`, non-nullable ones `type`; a non-nullable property with no schema default is *uninitialized* until assigned, so reading it before denormalization (e.g. on a hand-built `new Model()`) throws an `Error` — normalization treats it as absent. The `$initialized` bookkeeping array and the public `isInitialized()` method are gone: normalizers derive presence from `get_object_vars()`, preserving the distinction between a property never set and one explicitly set to `null`. The `AdditionalAndPatternProperties` runtime trait is now property-based (`definedProperties(): [php name => wire name]`) and merges additional properties with a key-preserving `+` (fixing numeric map keys such as HTTP status codes that `array_merge` previously renumbered). **Migration:** replace `$model->getFoo()` with `$model->foo`, `$model->setFoo($v)` with `$model->foo = $v`, and `$model->isInitialized('foo')` with `array_key_exists('foo', get_object_vars($model))`; extension containers keep `foreach`, `ArrayAccess`, `count()`, `toArray()`, `getArrayCopy()` and `json_encode()` unchanged
- **Review needed** [OpenApi] The emitted runtime `Client::executeEndpoint()` now declares a `: mixed` return type, `Client::create()` hints its first argument as `?Psr\Http\Client\ClientInterface`, and the emitted `Runtime\Client\Client` constructor promotes its four injected services into `protected readonly` properties. These are the one soft-BC-risk area of this pass: userland code overriding `executeEndpoint()` without a return type, or calling `create()` with a non-PSR-18 argument, will now get a PHP error instead of silently proceeding
- [JsonSchema] [OpenApi] Emitted runtime templates modernized: the `CheckArray` filter callback is an arrow function, `BaseEndpoint` drops a redundant parenthesis pair in its multipart boundary, the authentication-scope copy loop in `Client` is collapsed into a guard on the scopes array itself, and the OpenAPI 3 / 3.1 `transformResponseBody` generator emits `$contentType !== null` instead of `is_null($contentType) === false`
- [OpenApi] Documented the by-design query-string behavior in `BaseEndpoint::getQueryString()`: resolved `null` options are sent as empty values (`?foo=`) rather than being dropped, keeping the emitted query string stable whether or not an optional parameter was provided. Parameters declaring an OpenAPI serialization style are unaffected (nulls are skipped there per the OpenAPI spec)
- [JsonSchemaRuntime] `@realpath()` error suppression removed from `Reference`; local-path containment now guards with `is_dir()` / `file_exists()` before resolving, keeping warnings off and behavior identical
...
- [OpenApi] [GH#1051](https://github.com/janephp/janephp/issues/1051) Path parameters constrained with a regex in the path template (e.g. `/cluster/{id:.+}`) no longer leak the constraint into generated PHP variable names (`string $id:.+` — a syntax error): the variable name is now sanitized (`string $id`) in endpoint constructors, client methods and docblocks, consistently for OpenAPI 2 / 3 / 3.1. URL building still replaces the raw `{id:.+}` placeholder
- [JsonSchema] [GH#1051](https://github.com/janephp/janephp/issues/1051) Schemas named `parent` or `self` no longer generate reserved PHP class names (`class Parent` is a fatal error): they are now prefixed like other reserved words (`_Parent`, `_Self`), consistently wherever class names are derived (models, endpoints, operation naming)
